use constellation::scanner::{scan_projects, default_root};
...
fn main() {
    ...
    println!("scanned {} projects, {} sessions in {:?}", projects.len(), sessions, dur);
    println!();
    println!("top 5 by recency:");
    for p in projects.iter().take(5) {
        println!("  {:50}  {:3} sessions  total {:5} msgs",
            p.display_path(),
            p.sessions.len(),
            p.total_messages(),
        );
    ...
    if let Some(p) = projects.first() {
        for s in p.sessions.iter().take(3) {
            ...
            println!("  {:8}  msgs={:4}  model={:25}  {}",
                &s.id[..s.id.len().min(8)], s.message_count, s.model, title);
        }
    }
}
